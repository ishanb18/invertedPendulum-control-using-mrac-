...
def main():
    # Define test configurations for the MRAC Learning Rate (Gamma)
    configs = [
        {"gamma": 1.0, "title": "Slow Learning (Gamma = 1.0)"},
        {"gamma": 5.0, "title": "Nominal (Gamma = 5.0)"},
        {"gamma": 20.0, "title": "Fast Learning (Gamma = 20.0)"},
        {"gamma": 100.0, "title": "Aggressive (Gamma = 100.0)"}
    ]
    
    # 3 Rows (Angle, Control, Energy) x 4 Columns (Configurations)
    fig, axes = plt.subplots(3, 4, figsize=(20, 12), sharex=True)
    
    # Fixed Plant Physical Parameters
    m = 1.0
    l = 1.0
    g_gravity = 10.0
    I = (1/3) * m * l**2
    E_des = m * g_gravity * l  # Maximum potential energy at upright position
    
    print("Simulating Trajectories for 4 different MRAC Learning Rates (Gamma)...")
    ...
    for col, config in enumerate(configs):
        ...
        # Note: We pass the specific gamma into the MRACController
        sim = PendulumSimulation(render=False, m=m, l=l)
        swing = SwingUpController(m=m, l=l)
        mrac = MRACController(gamma=gamma)
        ref = ReferenceModel()
        ...
        for i in range(750):  # Run for 37.5 seconds
            ...
            curr_E = 0.5 * I * theta_dot**2 + 0.5 * m * g_gravity * l * (1 + np.cos(theta))
    ...
    fig.suptitle('MRAC Performance Analysis Across Learning Rates ($\gamma$)', fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Leave space for suptitle
    
    output_filename = 'learning_rate_trajectory_analysis.png'
